...
// e disparar a atualização do status (Supabase + Shopify + Make.com + Sellflux).
//
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // (bloco isolado, não-bloqueante)
      // ============================================
      try {
        const { data: orderData } = await supabase
          .from("orders")
          .select(
            "customer_name, customer_email, customer_phone, order_amount, product_type, product_quantity, pet_sizes, device_type"
          )
          .eq("order_id", orderId)
          .single()
        ...
        if (orderData) {
          dispatchSellflux({
            ...
            payment_method: "pix",
            payment_status: "paid",
            data_pedido: paymentDate,
          }).catch(() => {})
        } else {
          console.warn(
            `[Webhook] ⚠️ Pedido ${orderId} não encontrado no Supabase para disparar Sellflux.`
          )
        }
      } catch (sellfluxErr) {
        console.error(
          `[Webhook] ❌ Erro ao disparar Sellflux para pedido ${orderId}:`,
          sellfluxErr
        )
      }
