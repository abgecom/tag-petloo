...
async function createPagarmeSubscription(
  ...
  const subscriptionData = {
    ...
    plan_id: getPlanIdByQuantity(quantity),
  ...
  console.log("Plan ID:", getPlanIdByQuantity(quantity), "(quantidade:", quantity, ")")
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const totalQuantity = body.items.reduce((acc, item) => acc + item.quantity, 0)
      subscription = await createPagarmeSubscription(customer.id, card.id, totalQuantity)
    } catch (error) {
